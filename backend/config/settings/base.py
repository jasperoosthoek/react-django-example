...
EMAIL_USE_TLS = not not env('DJANGO_EMAIL_USE_TLS', default=False)
EMAIL_USE_SSL = not not env('DJANGO_EMAIL_USE_SSL', default=False)
EMAIL_SENDER = env('DJANGO_EMAIL_SENDER', default=None)
...
ADMINS = [('''Jasper Oosthoek''', 'jasperoosthoek@hotmail.com')]
...
# https://django-allauth.readthedocs.io/en/latest/configuration.html
# Normally we would use "mandatory" because users can sign in. However, here the only way to create new users
# is by the admin. Users need to reset their password which is the same as verifying their email address
...
DJOSER = {
    ...
    'SERIALIZERS': {
        'token_create': 'users.api.serializers.CustomTokenCreateSerializer',
    },
    'PERMISSIONS': {
