...
class GamesList extends Component {
  ...
  render() {
    ...
    if (getGamesIsLoading || !gamesList) return <LoadingIndicator />;
    ...
    return <>
      <div style={{ margin: '10px' }}>
        ...
        <Button
          onClick={() => this.setState({ showMatchMakerModal: true })}
          style={{ marginLeft: '10px' }}
        >
          Match maker
        </Button>
        <a
          href={`${process.env.REACT_APP_BACKEND_URL}/game/ranking/`}
          role="button"
        ...
        <Row className="card-example d-flex flex-row overflow-auto">
          {Object.values(gamesList).map((game, key) =>
            <Card style={{ minWidth: '36rem', margin: '10px' }} key={key}>
              <Card.Body
                onClick={() => this.setState({ gameInEditModal: game })}
              >
                {game.name}
      ...
      <CreateEditModal
        ...
        onSave={newGame => this.props.createGame(
          newGame,
          { callback: () => this.setState({ showNewModal: false }) }
        )}
      />
      <CreateEditModal
        show={this.state.showMatchMakerModal}
        modalTitle="Match maker"
        loading={this.props.matchMakerIsLoading}
        onHide={() => this.setState({ showMatchMakerModal: false })}
        initialState={{
          date: moment().format("YYYY-MM-DD"),
          teams: [],
        }}
        ...
        onSave={match => this.props.matchMaker(
          match,
          { callback: () => this.setState({ showMatchMakerModal: false }) }
        )}
      />

      {this.state.gameInEditModal &&
        <CreateEditModal
          show={!!this.state.gameInEditModal}
          modalTitle="Modify game"
          loading={updateGameIsLoading}
          onHide={() => this.setState({ gameInEditModal: null })}
          initialState={this.state.gameInEditModal}
          formFields={formFields}
          validate={({ score_1, score_2 }) => (score_1 === null && score_2 === null) || (score_1 !== null && score_2 !== null)}
          onSave={game => this.props.updateGame(
            game,
            { callback: () => this.setState({ gameInEditModal: null }) }
          )}
      />}
    </>;
  }
